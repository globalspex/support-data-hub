...
import { byCompany } from '@/server/services/reportService';
import { parseFilters } from '@/server/services/reportFilters';

export const Route = createFileRoute('/api/reports/by-company')({
  server: {
    handlers: {
      GET: async ({ request }) => {
        try { await requireAdminFromRequest(request); } catch (r) { return r as Response; }
        try {
          const data = await byCompany(parseFilters(new URL(request.url).searchParams));
